...
function searchOnGoogleBooks(searchStr, cb) {
    const url = `https://www.googleapis.com/books/v1/volumes?q=${searchStr}`

    const googleBooks = storageService.loadFromStorage(searchStr) || []
    if (googleBooks.length > 0) {
        // ?good idea?
        const books = Object.assign({}, ...googleBooks)
        console.log('googleBooks from storage returning:', books);
        cb(books)
        return books
    }
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            ...
            console.log('AJAX return', ans);
            // need to edit items
            cb(ans)
            storageService.post(searchStr, ans)
        }
    }
...
// function saveBooksSearch(books, searchStr) {
// do same with id,
//     console.log('saveBooksSearch', searchStr);
//     return storageService.post(searchStr, books)
// }

function getById(bookId) {
    return storageService.get(BOOKS_KEY, bookId);
}
// ?should i do all of this in the storage-service?
function addReview(bookId, review) {
    return getById(bookId)
        .then((book) => {
            ...
            //  return the save, can i just return the getById from here?
            return save(book)
            // .then((book) => {   
            //     console.log('review saved', book);
            // })
        })
}
...
// could get rid of it, and implement in query
